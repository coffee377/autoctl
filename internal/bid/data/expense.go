...
type BidExpenseForm struct {
	*DingTalkWorkflowData
	...
	ApplyInstanceID *string // 申请实例 ID

	ProjectId   *string // V2 项目 ID
...
func (ef *BidExpenseForm) getApplyMappers() []oa.FieldMapper {
	return []oa.FieldMapper{
		...
		{ComponentId: "FormRelateField_7YS5XZOZRUDC", FieldName: "ApplyInstanceID", Converter: ef.applyInstanceIdConverter, ExtValue: true, Pointer: true},
		...
		// 收款方信息 v1
		{ComponentId: "TextField_138818FKKQC00", FieldName: "PayeeName", Converter: oa.StringConverter, Version: "v1"},
		{ComponentId: "TextField_1HHDR2YRQGPS0", FieldName: "PayeeBank", Converter: oa.StringConverter, Version: "v1"},
		{ComponentId: "TextField_WKU63CA5OGG0", FieldName: "PayeeAccount", Converter: oa.StringConverter, Version: "v1"},
		// 收款方信息 v2
		{ComponentId: "RecipientAccountField_1FXC118HK3MDC", FieldName: "RecipientAccount", Converter: ef.recipientAccountConverter, ExtValue: true, Version: "v2", Pointer: true},
...
func (ef *BidExpenseForm) create(ctx context.Context, tx *ent.Tx) (*ent.BidExpense, error) {
	...
	// v2 收款组件存在
	if ef.RecipientAccount != nil {
		// 获取收款银行信息 todo 逻辑可能有问题，需要根据实际情况调整
		ef.PayeeBank = ef.RecipientAccount.InstBranchName
		ef.PayeeName = ef.RecipientAccount.Name
...
func (ef *BidExpenseForm) recipientAccountConverter(raw string, pointer bool) (interface{}, error) {
	var ra schema.RecipientAccount
	_ = json.Unmarshal([]byte(raw), &ra)
	if pointer {
		return &ra, nil
	}
	return ra, nil
...
func (f formRelateField) GetInstanceId() string {
	if f.Quote == 1 {
		return f.List[0].InstanceId
	}
	return ""
}

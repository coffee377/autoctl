...
func (BidExpense) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("guarantee_denomination").Comment("保函面额（元）").Default(0).SchemaType(
			map[string]string{
				dialect.MySQL: "decimal(16,2)",
			},
		),
		field.Time("guarantee_deadline").Comment("保函期限").Optional().Nillable().
		...
		field.Time("plan_pay_time").Comment("计划（最迟）转账时间").Optional().Nillable().
			SchemaType(map[string]string{
				dialect.MySQL: "datetime",
			}),
		field.Time("pay_time").Comment("付款时间").Optional().Nillable().
			SchemaType(map[string]string{
		...
		field.Text("remark").Comment("备注").Optional().Nillable(),
	}
}
